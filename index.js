...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        gameCard.innerHTML = `
            ...
            <p>This game has pledged $${games[i].pledged} from ${games[i].backers} donors </p>
        `;
...
const numBackers = GAMES_JSON.reduce( (acc, game) => {
    return acc + game.backers;
}, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `
    ${numBackers.toLocaleString('en-US')}
`;
...
const displayStr = `
    A total of $${totalAmountRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} ${numUnfundedGames == 1 ? "game" : "games"}.
